...
async function run({ targetPath, flags }) {
    ...
    if (sub === "status") {
        ...
        if (!initialized) {
            console.log(`   ${colors.red}❌ O projeto não possui o OpenClaw inicializado.${colors.reset}`);
            console.log(`   Rodar: npx openclaw init --apply primeiro.`);
            return;
        }
        console.log(`   ${colors.green}✔ Gateway simulado invocado com sucesso.${colors.reset}`);
        console.log(`   🔸 (O Web Server real porta 8000 deve ser atrelado a essa exec futuramente)`);
